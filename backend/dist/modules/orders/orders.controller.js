...
let OrdersController = class OrdersController {
    ...
    async findOne(id, req) {
        ...
        if (req.user.role !== user_entity_1.UserRole.ADMIN && order.user.id !== req.user.id) {
            throw new common_1.ForbiddenException('You do not have permission to view this order');
        }
        return order;
    }
    async confirmOrder(id, body, req) {
        return this.ordersService.confirmOrder(id, body.otp, req.user.id);
    }
    async updateStatus(id, updateOrderStatusDto) {
        return this.ordersService.updateStatus(id, updateOrderStatusDto.status, updateOrderStatusDto.otp);
    }
};
