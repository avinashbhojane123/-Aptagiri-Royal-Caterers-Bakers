...
@Injectable()
export class CaterersService implements OnModuleInit {
  constructor(
    @InjectRepository(CatererMenu)
    private catererRepository: Repository<CatererMenu>,
  ) {}
  ...
  async onModuleInit() {
    ...
    if (count === 0) {
      ...
      const defaultItems: {
        category: string;
        items: { name: string; price: number; desc: string }[];
      }[] = [
        {
        ...
        },
        {
          ...
          items: [
            ...
            }, // Temporary fix for dynamic keys if needed, but we'll use name
            {
              name: 'Modak',
              price: 3.99,
      ...
      for (const catGroup of defaultItems) {
        for (const item of catGroup.items) {
          const newItem = this.catererRepository.create({
            ...
            price: item.price,
            imageUrl: this.getImageUrlForCatererItem(
              item.name,
              catGroup.category,
            ),
          });
      ...
      console.log(
        'Catering items exist. Ensuring all items have valid imageUrl...',
      );
      const existingItems = await this.catererRepository.find();
      let updatedCount = 0;
      for (const item of existingItems) {
        if (!item.imageUrl) {
          item.imageUrl = this.getImageUrlForCatererItem(
            item.itemName,
            item.category,
          );
      ...
      if (updatedCount > 0) {
        console.log(
          `Updated ${updatedCount} existing catering items with image URLs.`,
        );
      }
    }
  }
  ...
  getImageUrlForCatererItem(name: string, category: string): string {
    ...
    // Welcome Drinks
    if (
      itemNameLower.includes('lemon mint') ||
    ...
    if (
      itemNameLower.includes('lemon ginger') ||
      itemNameLower.includes('ginger')
    ) {
      return 'https://images.unsplash.com/photo-1513558161293-cdaf765ed2fd?w=600&auto=format&fit=crop&q=80';
    }
    if (
      itemNameLower.includes('mocktail') ||
    ...
    if (
      itemNameLower.includes('manchurian') ||
    ...
    if (
      itemNameLower.includes('paneer tikka') ||
    ...
    if (
      itemNameLower.includes('paneer pakora') ||
  ...
  async getMenu(): Promise<CatererMenu[]> {
    return this.catererRepository.find({
      order: { category: 'ASC', itemName: 'ASC' },
    });
  }

  async getMenuByCategory(category: string): Promise<CatererMenu[]> {
    return this.catererRepository.find({
      where: { category },
      order: { itemName: 'ASC' },
    });
  ...
  async findOne(id: string): Promise<CatererMenu> {
    const item = await this.catererRepository.findOne({ where: { id } });
    if (!item) {
      throw new NotFoundException(`Caterer menu item with ID ${id} not found`);
    }
    return item;
  }

  async addItem(createDto: CreateCatererItemDto): Promise<CatererMenu> {
  ...
  async updateItem(
    id: string,
    updateDto: CreateCatererItemDto,
  ): Promise<CatererMenu> {
